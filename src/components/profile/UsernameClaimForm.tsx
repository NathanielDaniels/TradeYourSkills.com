...
export default function UsernameClaimForm({
  ...
  const sanitized = useMemo(
    () => sanitizeUsername(rawInput),
    [rawInput, sanitizeUsername]
  );
  const validationError = useMemo(
    () => validateUsername(sanitized),
    [sanitized, validateUsername]
  );
  ...
  const getStatusMessage = () => {
    ...
    if (sanitized && sanitized === currentUsername)
      ...
      {rateLimit && (
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3">
          <div className="flex items-center gap-2 text-sm text-blue-800 dark:text-blue-200">
            ...
            <span>
              {rateLimit.remaining} of {rateLimit.total} username changes
